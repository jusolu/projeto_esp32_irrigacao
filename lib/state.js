...
export async function getConfigAsync() {
  ...
  return {
    durationSec: Number(config?.durationSec) || 45,
...
export async function setConfigAsync(newConfig) {
  ...
  const updated = {
    ...current,
    ...newConfig,
    durationSec: Number(newConfig.durationSec) || current.durationSec || 45
  };
...
export async function getStateAsync() {
  ...
  return {
    ...
    config: config || { durationSec: 45, autoScheduleEnabled: true }
  };
}
...
export async function recordWateringEvent(eventData) {
  ...
  const newEntry = {
    ...
    durationSec: Number(eventData.durationSec) || state.config?.durationSec || 45,
