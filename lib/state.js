...
export async function recordWateringEvent(eventData) {
  ...
  const v = Number(eventData.batteryVoltage) || 3.7;
  const calcPct = Math.min(100, Math.max(0, Math.round(((v - 3.20) / (4.20 - 3.20)) * 100)));
  const pct = (eventData.batteryPct !== undefined && eventData.batteryPct !== null && eventData.batteryPct !== 94)
    ? Number(eventData.batteryPct)
    : calcPct;
  ...
  const newEntry = {
    ...
    batteryVoltage: Number(v.toFixed(2)),
    batteryPct: pct,
