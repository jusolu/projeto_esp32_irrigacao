// Manager de Estado, Configuração Dinâmica e Histórico de Irrigação no Upstash Redis
...
export async function getConfigAsync() {
  const config = await redisGetJson(KEY_CONFIG);
  return {
    durationSec: Number(config?.durationSec) || 60,
    autoScheduleEnabled: config?.autoScheduleEnabled ?? true
  };
}

export async function setConfigAsync(newConfig) {
  const current = await getConfigAsync();
  const updated = {
    ...current,
    ...newConfig,
    durationSec: Number(newConfig.durationSec) || current.durationSec || 60
  };

  await redisPipeline([
    ['SET', KEY_CONFIG, JSON.stringify(updated)]
  ]);
...
export async function getStateAsync() {
  const [water, history, config] = await Promise.all([
    redisGet(KEY_WATER),
  ...
  return {
    ...
    config: config || { durationSec: 60, autoScheduleEnabled: true }
  };
}
...
export async function recordWateringEvent(eventData) {
  ...
  const newEntry = {
    ...
    durationSec: Number(eventData.durationSec) || state.config?.durationSec || 60,
    source: eventData.source || 'RTC Agendado',
    actuators: eventData.actuators || 'Bomba (GPIO 4) + Válvula (GPIO 16)',
